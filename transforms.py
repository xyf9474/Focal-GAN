...
__all__ = [
    ...
    # custom classes
    "Grayscale_img_and_seg",
    "Resize_img_and_seg",
    "RandomCrop_img_and_seg",
    "RandomHorizontalFlip_img_and_seg",
    "Normalize_img_and_seg",
    "ToTensor_img_and_seg",
    "Compose_img_and_seg"
]
...
class ToTensor_img_and_seg:
    """Convert two ``PIL Image`` or ``numpy.ndarray`` to tensor. This transform does not support torchscript.

    Converts two PIL Image or numpy.ndarray (H x W x C) in the range
    [0, 255] to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0]
    if the PIL Image belongs to one of the modes (L, LA, P, I, F, RGB, YCbCr, RGBA, CMYK, 1)
    or if the numpy.ndarray has dtype = np.uint8
    ...
    def __call__(self, pic, pic_seg):
        ...
        Returns:
            Tensor: Converted image.
        """
        return F.to_tensor(pic),F.to_tensor(pic_seg)
...
class Normalize_img_and_seg(torch.nn.Module):
    """Normalize a tensor image with mean and standard deviation.
    This transform does not support PIL Image.
    Given mean: ``(mean[1],...,mean[n])`` and std: ``(std[1],..,std[n])`` for ``n``
    channels, this transform will normalize each channel of the input
    ...
    def forward(self, tensor: Tensor,tensor_seg: Tensor) -> Tensor:
        """
        Args:
            tensor (Tensor): Tensor image to be normalized.
...
class Resize_img_and_seg(torch.nn.Module):
    """Resize the input image and seg image to the given size using different interpolation method.
    If the image is torch Tensor, it is expected
    to have [..., H, W] shape, where ... means an arbitrary number of leading dimensions
    ...
    def __init__(self, size, 
                 ...
                 interpolation_seg=InterpolationMode.NEAREST, 
        ...
        if isinstance(interpolation_seg, int):
            warnings.warn(
                "Argument 'interpolation_seg' of type int is deprecated since 0.13 and will be removed in 0.15. "
                "Please use InterpolationMode enum."
            )
            interpolation_seg = _interpolation_modes_from_int(interpolation_seg)

        self.interpolation = interpolation
        self.interpolation_seg = interpolation_seg
        self.antialias = antialias
    ...
    def forward(self, img,seg):
        ...
        """
        return F.resize(img, self.size, self.interpolation, self.max_size, self.antialias),\
               F.resize(seg, self.size, self.interpolation_seg, self.max_size, self.antialias)
...
class RandomCrop_img_and_seg(torch.nn.Module):
    """Crop the given image and seg image at a random location,and img size must be same with seg
    If the image is torch Tensor, it is expected
    to have [..., H, W] shape, where ... means an arbitrary number of leading dimensions,
    but if non-constant padding is used, the input is expected to have at most 2 leading dimensions
...
class RandomHorizontalFlip_img_and_seg(torch.nn.Module):
    """Horizontally flip the given image randomly with a given probability.
    If the image is torch Tensor, it is expected
    to have [..., H, W] shape, where ... means an arbitrary number of leading
    dimensions
...
class Grayscale_img_and_seg(torch.nn.Module):
    """Convert image to grayscale and receive two inputs（img and seg）.
    If the image is torch Tensor, it is expected
    to have [..., 3, H, W] shape, where ... means an arbitrary number of leading dimensions
    ...
    def forward(self, img , seg):
        ...
        """
        return F.rgb_to_grayscale(img, num_output_channels=self.num_output_channels), \
               F.rgb_to_grayscale(seg, num_output_channels=self.num_output_channels)
